...
$d = date('Y-m-d');
$result = mysqli_query($con, "SELECT count(*) FROM repayments WHERE Status='Active' AND Recovery_Status = 'Yes' ");
...
<?php 
include '../config/db.php';
$result = mysqli_query($con, "SELECT SUM(Total_Bal) FROM repayments WHERE Status = 'Active' AND Recovery_Status = 'Yes'");
$row = mysqli_fetch_array($result);
$total = $row[0];
...
include '../config/db.php';
$d = date('Y-m-d');
$result = mysqli_query($con, "SELECT SUM(Amount) FROM recover WHERE Status = 'Paid'");
$row = mysqli_fetch_array($result);
$total = $row[0];
echo number_format($total,2);
?>
...
include '../config/db.php';
$d = date('Y-m-d');
$result = mysqli_query($con, "SELECT SUM(Amount) FROM recover WHERE  Date_Pay = '$d'");
$row = mysqli_fetch_array($result);
$total = $row[0];
echo number_format($total,2);
?>
