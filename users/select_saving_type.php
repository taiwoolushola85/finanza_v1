...
if ($savingsType === 'Flexi') {
    ...
    Withdraw_Savings, Savings_Repayment, Savings_Transfer, Savings_Upfront, Balance, Savings_Recieved FROM savings WHERE $whereClause ORDER BY id DESC LIMIT ? OFFSET ?";
    $stmt = mysqli_prepare($con, $dataQuery);
    $finalParams = array_merge($params, [$maxRows, $offset]);
    mysqli_stmt_bind_param($stmt, $paramTypes . "ii", ...$finalParams);
    mysqli_stmt_execute($stmt);
...
<div id="table-container" style="height:330px; overflow:auto">
    <table>
        <thead>
