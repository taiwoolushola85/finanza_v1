...
$nm = $_GET['nm'];
$d = date('Y-m-d');
$result = mysqli_query($con, "SELECT COUNT(*) FROM nip_notifications WHERE tnxdate = '$d' AND originatoraccountnumber LIKE '%$nm%' OR craccount LIKE '%$nm%' 
OR originatorname LIKE '%$nm%'");
...
<i>
<img src="loader.gif" style="height:20px"> Deleting Data ! Please wait...
</i>
</div>
</center>
</div>
...
if($gr == 'Super User'){
?>
<button type="button" class="btn btn-outline-danger btn-sm w-100" id="decline">
...
<div class="col-sm-2" style="margin-bottom:10px;">
<button type="button" class="btn btn-outline-primary btn-sm w-100" onclick="loads()" style="float:right">
...
include '../config/db.php';
$d = date('Y-m-d');
$result = mysqli_query($con, "SELECT DISTINCT id, originatoraccountnumber, amount, originatorname, craccountname, bankname, sessionid, craccount, Status,
bankcode, tnxdate, tnxtime, Branch, Officer_Name FROM nip_notifications WHERE tnxdate = '$d' AND originatoraccountnumber LIKE '%$nm%' OR craccount LIKE '%$nm%' 
OR sessionid LIKE '%$nm%' ORDER BY tnxdate ASC") or die("Bad Query.");
...
$fp = fopen('../data/search_bank_notification.json', 'w'); 
...
<thead>
<tr>
<th style="font-size:8px">MARK</th>
...
<tbody>
<?php
$url = '../data/search_bank_notification.json';
...
<td >
<?php 
if($member->Status == 'Notification Used'){
echo "<span style='color:red'>Notification Used</i>";
}else{
echo "<span style='color:green'>Notification Available</i>";
}
?></td>
...
$.ajax({
method: "GET",
url: "search_notification.php?nm=<?php echo $nm; ?>",
dataType: "html",
success:function(data){
$("#lds").hide();
$("#search").show();
...
WRN_PROFILE_DELETE = "Are you sure you want to delete this notification.?";
