...
<?php 
include '../config/db.php';
$result = mysqli_query($con, "SELECT count(*) FROM branch WHERE Status = 'Active'");
$row = mysqli_fetch_array($result);
$total = $row[0];
echo $total;
mysqli_close($con);
...
<span class="text-dark">Online User</span>
...
<?php
include '../config/db.php';
$Query = "SELECT  * FROM users WHERE Active = 'Online' ORDER BY id DESC LIMIT 8";
...
?>
<img class="rounded-circle bg-soft-primary img-fluid avatar-40 mb-2" src="<?php echo $imgPath; ?>"  loading="lazy">
<?php
}
}
mysqli_close($con);
...
$name= $rows['Name'];
?>
<option value="<?php echo $uzs; ?>"><?php echo $name; ?></option>
<?php
}
}
mysqli_close($con);
...
<div class="form-group">
<input type="text" placeholder="Enter New Password" class="select2-basic-single-tag js-states form-control" name="Amount" style="width: 100%;">
</div>
</div>
<button class="btn btn-primary btn-sm w-100 mt-2">Reset User</button>
</div>
</div>
</div>
</div>
